...
@interface TrendingAudioViewController () {
...
@implementation TrendingAudioViewController
...
- (void) getSongList {
    NSString *requestURL = [NSString stringWithFormat:@"%@%@=%@",ROOT_URL,@"method",@"trendingaudiolist"];
    [HttpAPI sendLoginRequest:NO url:requestURL completionBlock:^(BOOL success, NSData *resultData, NSError *err) {
        if (resultData != nil) {
            ...
            if (json != nil) {
                NSArray* songlist = [json objectForKey:@"trendingaudiolist"];
                if (songlist != nil) {
                    for (int i=0; i < [songlist count]; i++)
                    {
                        ...
                        NSString *poster = [item objectForKey:@"image"];
                        NSString *likecount = [NSString stringWithFormat:@"%d",0];
                        ...
                        //                        if ([type  isEqual: @"Trending"]) [trendinglist addObject:info];
                        //                        else if ([type  isEqual: @"Top10"]) [top10list addObject:info];
                        //                        else if ([type  isEqual: @"StaffPicker"]) [staffpicklist addObject:info];
                        //                        else if ([type  isEqual: @"Classic"]) [classiclist addObject:info];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (songList != nil && [songList count] > [indexPath row]) {
        ...
        cell.songInfo = info;
        //        [cell.playButton setBackgroundImage: [UIImage imageNamed:@"playbutton.png"] forState:UIControlStateNormal ];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (songList && songList.count > 0) {
        ...
        [self increaseCount:0 playcount:1 ListSongCell:(ListSongViewCell*)[tableView cellForRowAtIndexPath:indexPath]];
...
- (void)swipeableTableViewCell:(SWTableViewCell *)cell didTriggerLeftUtilityButtonWithIndex:(NSInteger)index
{
    ...
    switch (index) {
        case 0:
            [self increaseCount:1 playcount:1 ListSongCell:listsongCell];
